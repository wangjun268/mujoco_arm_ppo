...
drop in a new URDF and re-run ``python3 tools/convert_hand_urdf.py``.
"""
...
# Allow `python3 tools/convert_hand_urdf.py` as well as `python3 -m tools.convert_hand_urdf`.
...
def standalone_model(meshdir: str = "../") -> str:
  ...
  <!-- Standalone preview of the vendored L20.  Generated by
       tools/convert_hand_urdf.py; do not edit by hand. -->
...
class GraspCalibration:
    ...
    * the **grasp centre** -- the point the scene's resolved-rate expert servos
    ...
    approach is handled by the expert in ``grasp.common``.
    """
